...
| Rate Limiting | Planned: rate limiting for login and public/integration endpoints (`lib/security/rateLimit.ts`). **Note:** Next.js 16 renamed root `middleware.ts` to `proxy.ts` (export `proxy`) for route/session checks; do not add a `middleware.ts` for that role. |
