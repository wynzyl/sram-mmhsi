...
/**
 * Card header row for Invoice Queue with inline controls.
 * Renders inside the card section - page title is in the parent template.
 */
export default function InvoiceQueueHeader({
  basePath,
  gradeLevels,
  totalCount,
  canSend,
}: InvoiceQueueHeaderProps) {
    ...
    <>
      ...
      <div className="flex flex-col gap-3 border-b border-border px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div role="search" className="flex items-center gap-2">
          ...
          <Link
            href={`${basePath}/batch`}
            className="btn-secondary min-h-10 px-4 whitespace-nowrap inline-flex items-center gap-2"
          >
            <FileStack className="h-4 w-4" />
            <span className="hidden lg:inline">Batch Generate</span>
          </Link>

          {/* Batch Send Button */}
          {canSend && (
            <Link
              href={`${basePath}/batch-send`}
              className="btn-primary min-h-10 px-4 whitespace-nowrap inline-flex items-center gap-2"
            >
              <Mail className="h-4 w-4" />
              <span className="hidden lg:inline">Batch Send</span>
            </Link>
          )}
        </div>
      </div>
