...
function registrationsListHref(opts: { schoolYearId?: string; page?: number }) {
  ...
  const s = p.toString();
  return s ? `/staff/registrations?${s}` : "/staff/registrations";
}

export default async function StaffRegistrationsPage({ searchParams }: PageProps) {
  const session = await requireSession();
  if (!hasPermission(session.role, "registrations:read")) redirect("/staff/dashboard");
  ...
  return (
    <div className="page-container">
      ...
      >
        ...
        {schoolYearId != null && (
          <Link href="/staff/registrations" className="btn-ghost text-sm">
            Clear filter
          </Link>
        )}
      </form>
