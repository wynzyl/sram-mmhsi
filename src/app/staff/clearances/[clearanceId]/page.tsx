...
export default async function StaffClearanceDetailPage({ params }: PageProps) {
  ...
  if (!hasPermission(session.role, "clearances:read")) {
    redirect("/staff/approvals?section=clearance&error=no_permission");
  }
  ...
  const canResolve = hasPermission(session.role, "clearances:resolve");
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Resolution Form (for pending + has permission) or Resolution Details */}
      {isPending ? (
        canResolve ? (
          ...
          <Card>
            <CardContent className="py-6">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <p className="text-sm">
                  This clearance is pending resolution. You do not have permission to resolve clearances.
                </p>
              </div>
            </CardContent>
          </Card>
