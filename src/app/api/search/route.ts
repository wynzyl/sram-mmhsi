...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Execute all permitted queries in parallel for better performance
    const [studentResults, enrollmentResults, assessmentResults, paymentResults] =
      await Promise.all([
        // Search students
        ...
        // Search enrollments
        hasPermission(user.role, "enrollments:read")
          ? db
              .select({
                id: enrollments.id,
        ...
        // Search assessments
        hasPermission(user.role, "assessments:read")
          ? db
              .select({
                id: assessments.id,
        ...
        // Search payments
        hasPermission(user.role, "payments:read")
          ? db
              .select({
                id: payments.id,
    ...
    const response: GlobalSearchResponse = {
      students: studentResults.map((s): SearchResultItem => ({
        id: s.id,
      ...
      assessments: assessmentResults.map((a): SearchResultItem => ({
        id: a.id,
      ...
      payments: paymentResults.map((p): SearchResultItem => ({
        id: p.id,
