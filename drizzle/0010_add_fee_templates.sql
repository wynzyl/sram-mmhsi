-- Migration: Add Fee Templates Architecture
-- Adds new template-based fee schedule system while preserving existing data

-- Create fee_item_type_category enum
CREATE TYPE "public"."fee_item_type_category" AS ENUM('tuition', 'fees', 'materials', 'discount', 'other');

-- Create fee_item_types table (master fee definitions)
...
-- Create fee_template_items table
...
-- Create school_year_fee_schedules table
...
-- Create fee_schedule_overrides table
...
-- Add new columns to assessment_items
ALTER TABLE "assessment_items" ADD COLUMN "fee_template_item_id" uuid;
ALTER TABLE "assessment_items" ADD COLUMN "fee_item_type_id" uuid;

-- Add foreign keys for fee_item_types
...
-- Add foreign keys for fee_templates
...
-- Add foreign keys for fee_template_items
...
-- Add foreign keys for school_year_fee_schedules
...
-- Add foreign keys for fee_schedule_overrides
...
-- Add foreign keys for assessment_items new columns
...
-- Create indexes for fee_item_types
...
-- Create indexes for fee_template_items
...
-- Create indexes for school_year_fee_schedules
...
-- Create indexes for fee_schedule_overrides
...
-- Create indexes for assessment_items new columns
